from plane import Dot
import numpy as np
from math import sqrt, pow


# Dot projection on plane computing part
def getDotProjectionOnPlane(dot_A, a, b, c, d):
...
def cramerMethod(dot_A, a, b, c, d):
...
def getDotProjectionOnLine(dot_A, dot_line_a, dot_line_b):
    """ Get dot_A projection on a line, defined by dot_line_a and dot_line_b """
    a_top, a_bot, b_top, b_bot, c_top, c_bot = computeLineCoefficents(dot_line_a, dot_line_b)
    x, y, z = dotProjectionOnLine(dot_A, a_top, a_bot, b_top, b_bot, c_top, c_bot)
    return Dot(x, y, z)


def dotProjectionOnLine(dot_A, a_top, a_bot, b_top, b_bot, c_top, c_bot):
...
def computePlaneCoefficientsWithNormalVector(dot_A, a, b, c):
...
# Line computing part
def computeLineCoefficents(dot_A, dot_B):
...
def getDistanceBetweenDots(dot_A, dot_B):
